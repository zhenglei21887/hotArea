...
(function ($) {
	...
	HotArea.prototype = {
		...
		init: function ($ele, options) {
			...
			$canvas
					...
					.offset({
						top: this.$ele.position().top + (parseInt(this.$ele.css("border-top")) || 0),
						left: this.$ele.position().left + (parseInt(this.$ele.css("border-left")) || 0),
					})
		...
		draw: function () {
			...
			$.each(this.areas, function(index, area) {console.log(area)
		...
		toTag: function () {
			...
			validAreas.forEach(function(item) {
				tagHtmlArr.push(String()
					...
					+			'left: '+item.x*100/as.$canvas.width()+'%;'
					+			'top: '+item.y*100/as.$canvas.height()+'%;'
					+			'height: '+item.height*100/as.$canvas.height()+'%;'
					+			'width: '+item.width*100/as.$canvas.width()+'%;'
					+			'">'
					+		'<a href="'+item.url+'"></a>'
					+ '</div>'
				);
			});
			tagHtmlArr.push(as.$ele.find("img")[0].outerHTML);

			as.linkUpdateStatus = linkUpdateStatus.HIDE;
			as.linkUpdate();
			$hotCover.html(tagHtmlArr.join("")).show();
			var rtnHtml = as.$ele.find("style")[0].outerHTML + $hotCover[0].outerHTML;
			$hotCover.hide();
		...
		fromTag: function (tag) {
			...
			as.$ele.html($(tag).find("img")[0].outerHTML).find("img").on("load", function() {
				$(tag).find("div").each(function(index, dom) {
					var $item = $(dom);
					areas.push({
						x: parseFloat($item.css("left")||$item.css("margin-left"))/100*as.$ele.find("img").width(),
						y: parseFloat($item.css("top")||$item.css("margin-top"))/100*as.$ele.find("img").height(),
						width: parseFloat($item.css("width"))/100*as.$ele.find("img").width(),
						height: parseFloat($item.css("height"))/100*as.$ele.find("img").height(),
			...
			if(as.currentArea) {
				$linkUpdate
						...
						.unbind("change").change(function() {
							...
							if(linkVal.indexOf("http://") !== 0 && linkVal.indexOf("https://") !== 0 && linkVal.indexOf("//") !== 0) {
								linkVal = "//" + linkVal;
							}
	...
	$.fn.hotArea = function (method) {
		...
		if (!as && (method === undefined || $.isPlainObject(method))) {
			...
			if (as === undefined) {
				...
				var rtn = as[method](...Array.from(arguments).slice(1));
				return method === "toTag" ? rtn : as.$ele;
			} else {
